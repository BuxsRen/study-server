...
// 创建 WebSocket 服务,传入该服务的唯一ID，单个区域最大的人数
/**
*@Example:
...
// 客户端升级 WebSocket 并绑定相关事件
func (ws *WebSocket) Bind(content *gin.Context, i WebSocketInterface) (*Client, error) {
	if content.GetString("_id") == "" {
		return &Client{}, errors.New("用户ID解析失败")
	}
	conn, e := upGrader.Upgrade(content.Writer, content.Request, nil) //客户端连接，升级get请求为webSocket协议
	if e != nil {
		return &Client{}, e
	}
	c := &Client{Coon: conn, server: ws, Id: content.GetString("_id"), i: i}
	c.msgChan = make(chan []byte, 256)
	c.route = make(map[string]func(*Client, []byte))
	for _, v := range i.Route() {
		c.route[v.Action] = v.Fun
	}
	c.Rpc = content.GetString("_rpc")
	c.Group = content.GetString("_group")
	c.Udp = content.GetString("_udp")
	if c.Group == "" {
		c.Group = "area"
	}
	return c, nil
...
// 开始对客户端监听并提供后续的功能，使用分组，传入分组名称或者不使用传空字符串
...
// 取群组中所有区域 map[string][]*area.Client
...
// 给在线的所有客户端推送消息 可忽略掉指定客户端，传入客户端的id即可
